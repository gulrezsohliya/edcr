...
public class BuildingHeight_AR extends BuildingHeight {
					...
					if (block.getBuilding() != null && (block.getBuilding().getBuildingHeight() != null
						...
						for (Floor floor : block.getBuilding().getFloors()) {
							if (floor.getParking().getStilts() != null && !floor.getParking().getStilts().isEmpty()) {
								for (Measurement s : floor.getParking().getStilts()) {
									if (s.getArea() != null) {
										stiltExist = Boolean.TRUE;
									}
								}
							}
						...
						if (mostRestrictiveOccupancyType != null) {
							if (mostRestrictiveOccupancyType.getType() != null
								...
								processBuildingHeightResidential(pl, mostRestrictiveOccupancyType, PlotArea, stiltExist,
								...
								processBuildingHeightCommercial(pl, mostRestrictiveOccupancyType, PlotArea, stiltExist,
										buildingHeight, maxBuildingHeight);
							else if (mostRestrictiveOccupancyType.getType() != null
									&& "I".equalsIgnoreCase(mostRestrictiveOccupancyType.getType().getCode()))
								processBuildingHeightIndustrial(pl, mostRestrictiveOccupancyType, PlotArea, stiltExist,
										buildingHeight, maxBuildingHeight);
							else if (mostRestrictiveOccupancyType.getType() != null
									&& "G".equalsIgnoreCase(mostRestrictiveOccupancyType.getType().getCode()))
								processBuildingHeightGovernmentUse(pl, mostRestrictiveOccupancyType, PlotArea,
										stiltExist, buildingHeight, maxBuildingHeight);
							else if (mostRestrictiveOccupancyType.getType() != null
									&& "T".equalsIgnoreCase(mostRestrictiveOccupancyType.getType().getCode()))
								processBuildingHeightTransportation(pl, mostRestrictiveOccupancyType, PlotArea,
										stiltExist, buildingHeight, maxBuildingHeight);
							else if (mostRestrictiveOccupancyType.getType() != null
									&& "P".equalsIgnoreCase(mostRestrictiveOccupancyType.getType().getCode()))
								processBuildingHeightPublicSemiPublic(pl, mostRestrictiveOccupancyType, PlotArea,
										stiltExist, buildingHeight, maxBuildingHeight);
							else {
	...
	private void processBuildingHeightIndustrial(Plan pl, OccupancyTypeHelper mostRestrictiveOccupancyType,
			BigDecimal PlotArea, Boolean stitExist, BigDecimal buildingHeight, BigDecimal maxBuildingHeight) {
	...
	private void processBuildingHeightCommercial(Plan pl, OccupancyTypeHelper mostRestrictiveOccupancyType,
			BigDecimal PlotArea, Boolean stiltExist, BigDecimal buildingHeight, BigDecimal maxBuildingHeight) {
		...
		if (mostRestrictiveOccupancyType.getSubtype().getCode().equals("C2a"))
			if (PlotArea.compareTo(PLOT_AREA_500) < 0) {
				pl.addError("PlotArea below 500 Hotel", "Plot area below 500 sqmts not allowed for hotel");
			} else if (PlotArea.compareTo(PLOT_AREA_500) >= 0 && PlotArea.compareTo(PLOT_AREA_1000) <= 0) {
				maxBuildingHeight = HEIGHT_AR_15;
			} else {
		...
		if (mostRestrictiveOccupancyType.getSubtype().getCode().equals("C5a"))
			return;
		if (mostRestrictiveOccupancyType.getSubtype().getCode().equals("C6a"))
			maxBuildingHeight = HEIGHT_AR_15;
		if (maxBuildingHeight.compareTo(BigDecimal.ZERO) > 0)
			buildResult(pl, this.scrutinyDetail, buildingHeight, maxBuildingHeight, "Zone",
					mostRestrictiveOccupancyType);
	...
	private void processBuildingHeightPublicSemiPublic(Plan pl, OccupancyTypeHelper mostRestrictiveOccupancyType,
			BigDecimal PlotArea, Boolean stitExist, BigDecimal buildingHeight, BigDecimal maxBuildingHeight) {
	...
	private void processBuildingHeightTransportation(Plan pl, OccupancyTypeHelper mostRestrictiveOccupancyType,
			BigDecimal plotArea, Boolean stitExist, BigDecimal buildingHeight, BigDecimal maxBuildingHeight) {
		if (mostRestrictiveOccupancyType.getSubtype().getCode().equals("I1b"))
			return;
		if (mostRestrictiveOccupancyType.getSubtype().getCode().equals("I1a")
	...
	private void processBuildingHeightGovernmentUse(Plan pl, OccupancyTypeHelper mostRestrictiveOccupancyType,
			BigDecimal plotArea, Boolean stitExist, BigDecimal buildingHeight, BigDecimal maxBuildingHeight) {
	...
	private void processBuildingHeightResidential(Plan pl, OccupancyTypeHelper mostRestrictiveOccupancyType,
			BigDecimal PlotArea, Boolean stiltExist, BigDecimal buildingHeight, BigDecimal maxBuildingHeight) {
		...
		if (mostRestrictiveOccupancyType.getSubtype().getCode().equals("R1b")
			...
			if (PlotArea.compareTo(PLOT_AREA_48) < 0) {
				pl.addError("PlotArea below 48 Residential",
						"Plot area below 48 sqmts not allowed for plotted residential housing");
		...
		if (maxBuildingHeight.compareTo(BigDecimal.ZERO) > 0)
			buildResult(pl, this.scrutinyDetail, buildingHeight, maxBuildingHeight, "3.4.3 ",
					mostRestrictiveOccupancyType);
	}

	private void buildResult(Plan pl, ScrutinyDetail scrutinyDetail, BigDecimal buildingHeight,
			BigDecimal maxBuildingHeight, String ruleNo, OccupancyTypeHelper mostRestrictiveOccupancyType) {
		...
		details.put("Occupancy", mostRestrictiveOccupancyType.getSubtype().getName());

...
@Service
public class ExitWidth_Manipur extends ExitWidth{
	private static final Logger LOG = Logger.getLogger(ExitWidth_Manipur.class);
				...
				for (Floor floor : block.getBuilding().getFloors()) {
					for(Door door:floor.getDoors()) {
						if(door.getWidths()!=null)
							...
							plan.addError("Width Door Not Found","Width of Door Not defined");
					}
					if(floor.getExitWidthDoor()!=null)
						exitwidthDoor=floor.getExitWidthDoor().stream().mapToDouble(BigDecimal::doubleValue).sum();
//					else
//						plan.addError("Exit Width Door Not Found","Exit Width Door Not defined");
				}

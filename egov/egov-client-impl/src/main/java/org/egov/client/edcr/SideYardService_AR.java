...
public class SideYardService_AR extends SideYardService {
							...
							if(sideYard1!=null && sideYard1.getMinimumDistance()!=null) {
								...
								occupancy.setTypeHelper(block.getBuilding().getMostRestrictiveFarHelper());
									...
									if (occupancy.getTypeHelper().getType() == null) {
										...
										if(buildingHeight.compareTo(BigDecimal.valueOf(15))>0 && stiltExist==Boolean.FALSE
												|| buildingHeight.compareTo(BigDecimal.valueOf(17.4))>0 && stiltExist==Boolean.TRUE) {
											sideYard1Result = checkSideYardForHighRise_Side1(pl, block.getBuilding(),
													 block.getName(), setback.getLevel(), plot,
													minlength, max,occupancy.getTypeHelper(), sideYard1Result, sideYard2Result,errors);
										
											sideYard2Result = checkSideYardForHighRise_Side2(pl, block.getBuilding(),
													 block.getName(), setback.getLevel(), plot,
													minlength, max,occupancy.getTypeHelper(), sideYard1Result, sideYard2Result,errors);
										}else {
	...
	private SideYardResult checkSideYardForHighRise_Side2(Plan pl, Building building, String blockName, Integer level,
		...
		String rule = "3.4.3";
		String subRule = "3.4.3";

		Boolean valid1 = false;


		BigDecimal	side2val=SIDEVALUE_THREE;
	...
	private SideYardResult checkSideYardForHighRise_Side1(Plan pl, Building building, String blockName, Integer level,
			Plot plot, double minlength, double max, OccupancyTypeHelper mostRestrictiveOccupancyType, SideYardResult sideYard1Result,
			SideYardResult sideYard2Result,HashMap<String, String> errors) {
		String rule = "3.4.3";
		String subRule = "3.4.3";
		...
		BigDecimal side1val = SIDEVALUE_THREE;
	...
	private void addSideYardResult(final Plan pl, HashMap<String, String> errors, SideYardResult sideYard1Result,
		...
		if (errors.isEmpty()) {
			...
			if (sideYard2Result != null) {
				...
				detailsSideYard2.put(RULE_NO, "3.4.3");

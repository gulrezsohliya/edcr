...
public class Coverage_AR extends Coverage {
	...
	private static final BigDecimal COVERAGE_AREA_75 = BigDecimal.valueOf(75);
	private static final BigDecimal COVERAGE_AREA_70 = BigDecimal.valueOf(70);
	private static final BigDecimal COVERAGE_AREA_66 = BigDecimal.valueOf(66);
	private static final BigDecimal COVERAGE_AREA_65 = BigDecimal.valueOf(65);
	private static final BigDecimal COVERAGE_AREA_60 = BigDecimal.valueOf(60);
	private static final BigDecimal COVERAGE_AREA_55 = BigDecimal.valueOf(55);
	private static final BigDecimal COVERAGE_AREA_50 = BigDecimal.valueOf(50);
	private static final BigDecimal COVERAGE_AREA_45 = BigDecimal.valueOf(45);
	private static final BigDecimal COVERAGE_AREA_40 = BigDecimal.valueOf(40);
	private static final BigDecimal COVERAGE_AREA_35 = BigDecimal.valueOf(35);
	private static final BigDecimal COVERAGE_AREA_33 = BigDecimal.valueOf(33);
	private static final BigDecimal COVERAGE_AREA_33_33 = BigDecimal.valueOf(33.33);
	private static final BigDecimal COVERAGE_AREA_30 = BigDecimal.valueOf(30);
	private static final BigDecimal COVERAGE_AREA_25 = BigDecimal.valueOf(25);
	private static final BigDecimal COVERAGE_AREA_20 = BigDecimal.valueOf(20);
	private static final BigDecimal COVERAGE_AREA_10 = BigDecimal.valueOf(10);
	...
	private void processCoveragePublicSemiPublic(Plan pl, OccupancyTypeHelper mostRestrictiveOccupancyType,
		...
		// Meeting Hall, Auditorium, Community Center
				if (mostRestrictiveOccupancyType.getSubtype().getCode().equals(P3a)) {
					requiredPlotArea = "-";
					coveragePercentage = COVERAGE_AREA_30;

				}
				
				if (mostRestrictiveOccupancyType.getSubtype().getCode().equals(P3b)) {
					requiredPlotArea = "-";
					coveragePercentage = COVERAGE_AREA_35;
				...
				if (mostRestrictiveOccupancyType.getSubtype().getCode().equals(P3c)) {
					if(PlotArea.compareTo(PLOT_AREA_100)<0) {
						...
						coveragePercentage = COVERAGE_AREA_45;
				...
				if (mostRestrictiveOccupancyType.getSubtype().getCode().equals(P1b)) {
					requiredPlotArea = "-";
					coveragePercentage = COVERAGE_AREA_40;
				...
				if (mostRestrictiveOccupancyType.getSubtype().getCode().equals(P1a)) {
					requiredPlotArea = "-";
					coveragePercentage = COVERAGE_AREA_25;
				}
				if(requiredPlotArea!="")
		...
		// Rail Terminal
				if (mostRestrictiveOccupancyType.getSubtype().getCode().equals(T1b)) {
					if (pl.getPlanInfoProperties().get("TRANSPORTATION_AREA_TYPE").equalsIgnoreCase("OPERATION")) {
						...
						requiredPlotArea = "-";
						coveragePercentage = COVERAGE_AREA_70;
		...
		// Flatted
				if (mostRestrictiveOccupancyType.getSubtype().getCode().equals(I1a)) {
					if(PlotArea.compareTo(PLOT_AREA_2000)<0) {
						...
						coveragePercentage = COVERAGE_AREA_33_33;
					}
	...
	private void processCoverageCommercial(Plan pl, OccupancyTypeHelper mostRestrictiveOccupancyType, BigDecimal PlotArea,
		...
		if (mostRestrictiveOccupancyType.getSubtype().getCode().equals(C1a)
			...
			if(PlotArea.compareTo(PLOT_AREA_48)<0) {
				...
				coveragePercentage = COVERAGE_AREA_75;
			} else if (PlotArea.compareTo(PLOT_AREA_100) > 0 && PlotArea.compareTo(PLOT_AREA_250) <= 0) {
				requiredPlotArea = "Above 100 to 250";
				coveragePercentage = COVERAGE_AREA_65;
			} else if (PlotArea.compareTo(PLOT_AREA_250) > 0 && PlotArea.compareTo(PLOT_AREA_500) <= 0) {
		...
		if (mostRestrictiveOccupancyType.getSubtype().getCode().equals(C5a)) {
			if (PlotArea.compareTo(PLOT_AREA_1080) > 0) {
				...
				coveragePercentage = COVERAGE_AREA_20;
			} else if (PlotArea.compareTo(PLOT_AREA_510) > 0) {
				requiredPlotArea = "Above 510";
				coveragePercentage =COVERAGE_AREA_10;
...
//		scrutinyDetail.addColumnHeading(3, "Coverage Area");
...
//		String actualResult = getLocaleMessage(RULE_ACTUAL_KEY, coverage.toString());
		...
		details.put(RULE_NO, "Chapter 3");
		details.put(DESCRIPTION, desc);
//		details.put("Coverage Area", coveredArea.setScale(2, RoundingMode.DOWN).toString());
		details.put(PERMISSIBLE,"<= "+ expectedResult.toString());
		details.put(PROVIDED, coveredArea.setScale(2, RoundingMode.DOWN).toString());
